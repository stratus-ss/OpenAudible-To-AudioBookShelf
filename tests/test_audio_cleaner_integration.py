...
class TestAudioCleanerInitialization:
    ...
    def test_initializes_statistics(self):
    ...
    def test_reads_copy_mode_from_config(self):
        """Test that copy mode is read from config."""
        config = Mock()
        config.working_directory = "/tmp/test"
        config.copy_instead_of_move = True
        log_file = Mock()
...
class TestSetupOutputPaths:
    ...
    def test_creates_book_specific_directory(self):
    ...
    def test_handles_m4b_format_conversion(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert paths["audio_format"] == "m4a"
            assert paths["ext"] == "m4b"

    def test_includes_transcript_path_when_enabled(self):
    ...
    def test_excludes_transcript_path_when_disabled(self):
...
class TestInitializeMonkeyplug:
    ...
    def test_creates_whisper_plugger_with_correct_params(self, mock_plugger_class):
        """Test that WhisperPlugger is initialized with correct parameters."""
        config = Mock()
        config.working_directory = "/tmp/test"
        config.swears_file = "/path/to/swears.txt"
        config.save_transcripts = True
    ...
    @patch('modules.audio_cleaner.WhisperPlugger')
    def test_reuses_existing_transcript(self, mock_plugger_class):
    ...
    def test_raises_audio_cleaning_error_on_failure(self, mock_plugger_class):
...
class TestProcessAudioFile:
    ...
    def test_uses_chunker_for_large_files(self, mock_chunker_class, mock_plugger_class):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Mock chunker to indicate chunking is needed
            mock_chunker_instance = MagicMock()
            mock_chunker_instance.needs_chunking.return_value = True
            mock_chunker_instance.process_with_chunking.return_value = output_file
            ...
            # Verify AudioChunker was created
            mock_chunker_class.assert_called_once()
            
            # Verify needs_chunking was called
            mock_chunker_instance.needs_chunking.assert_called_once_with(source_file)
            
            # Verify process_with_chunking was called
            mock_chunker_instance.process_with_chunking.assert_called_once()
    ...
    @patch('modules.audio_cleaner.WhisperPlugger')
    @patch('modules.audio_cleaner.AudioChunker')
    def test_uses_direct_encoding_for_small_files(self, mock_chunker_class, mock_plugger_class):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Mock chunker to indicate chunking is NOT needed
            mock_chunker_instance = MagicMock()
            mock_chunker_instance.needs_chunking.return_value = False
            mock_chunker_class.return_value = mock_chunker_instance
            ...
            mock_chunker_instance.process_with_chunking.assert_not_called()
    ...
    @patch('modules.audio_cleaner.WhisperPlugger')
    @patch('modules.audio_cleaner.AudioChunker')
    def test_returns_original_file_on_error(self, mock_chunker_class, mock_plugger_class):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            mock_chunker_class.side_effect = Exception("Chunker failed")
...
class TestCleanupWorkingDirectory:
    ...
    def test_keeps_directory_when_saving_transcripts(self):
    ...
    def test_preserves_files_in_copy_mode(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = Mock()
            config.working_directory = work_dir
            config.save_transcripts = False  # Even with False
            config.copy_instead_of_move = True  # Copy mode overrides
            log_file = Mock()
...
class TestLogStatistics:
    ...
    def test_writes_statistics_to_log(self):
