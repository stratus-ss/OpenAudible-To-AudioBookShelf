...
> [!IMPORTANT]
> **Project Rename In Progress:** This project is being renamed to **`Import-To-AudioBookShelf`**.
> During the transition you may still see the old name (`OpenAudible-To-AudioBookShelf`) in local paths, service names, and repository references.
> If you update clone paths, remotes, or automation, use the new project name going forward.
...
* **Multiple Download Sources:** Supports both OpenAudible and Libation as download sources (CLI usage). MCP server requires Libation only.
...
* **OpenAudible or Libation:** This script assumes you have either OpenAudible or Libation installed and configured. **For MCP server usage, only Libation is supported** — OpenAudible lacks the CLI automation needed for AI agent workflows.
...
* **--yaml:** Path to YAML configuration file. Can be combined with `--step` and `--asins`.
* **--step:** Run a single pipeline step instead of the full pipeline. Choices: `scan`, `download`, `export`, `organize`, `scan-abs`, `match`. Returns structured JSON output. Designed for LLM orchestration via the MCP server.
* **--asins:** Space-separated list of Audible product IDs (ASINs) to process. Used with `--step download` to download specific books.
