...
5. **ALL profanity-cleaning config is server-side. NEVER investigate it.** The swears file, Whisper backend URL, working directory, model selection, and all MonkeyPlug parameters are configured in the MCP server `.env` on `open-audible`. Do NOT:
   - Search for config files, `.env` files, or swears files locally on `arch-openclaw`
   - Try to SSH to remote servers to inspect config
   - Search for MonkeyPlug installation paths or `libraries.yaml`
   - Run `find` or `grep` for anything related to profanity/cleaning/monkeyplug config
   - Investigate why cleaning previously failed by looking at filesystem artifacts

   Just call `organize_books(library=..., enable_profanity_cleaning=true)`. If it fails, `get_job_result` returns the error. Report that error to the user verbatim. That is the ONLY debugging you do.
